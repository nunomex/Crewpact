/* ============================================================================
   CrewPact · Sistema de Ícones v1 — FONTE ÚNICA
   ----------------------------------------------------------------------------
   Todos os pictogramas da app vivem AQUI. Nenhuma página redesenha ícones:
   inclui este ficheiro e referencia um nome. Isto mata a deriva (a auditoria
   encontrou 8,9% de fidelidade — cada página reinventava).

   REGRAS (do canon icones-crewpact.html, spec letã RAL 9005/1033/9003):
   · SÓLIDOS, nunca contorno/stroke  · grelha 24  · cantos francos
   · fill = currentColor → a COR vem do CSS (`color:`) do contexto
     (amarelo sobre placa preta, ink sobre papel — controla-se com `color:`).

   USO (HTML, marcação declarativa):
     <i data-ic="lock"></i>
     <i data-ic="chevron" data-ic-rot="90"></i>      (chevron ⌄)
     <i data-ic="arrow-r" data-ic-size="16"></i>
   USO (JS, quando a página gera markup):
     el.innerHTML = CPIcons.icon('bed', { size:22, rot:0 });

   A hidratação corre sozinha no DOMContentLoaded. Nome inválido → quadrado
   laranja visível (erro que se vê, não falha silenciosa).

   NOMES (50) — ver CPIcons.names para a lista completa em runtime:
     base:    plane cal stats clock bed share fam wallet doc moon sun alert
              home gauge plus tower
     setas:   arrow-r arrow-u arrow-diag arrow-dots brackets
     aeroporto (ISO 7001): departs arrivs gate passport transfer info medical
              shuttle ticket restricted
     chrome:  chevron close back search trash edit check ellipsis mail lock
              bell download pin phone bulb logout eye user faceid
   ========================================================================== */
(function () {
  var PLANE = 'M21 16v-2l-8-5V3.5C13 2.7 12.33 2 11.5 2S10 2.7 10 3.5V9l-8 5v2l8-2.5V19l-2 1.5V22l3.5-1 3.5 1v-1.5L13 19v-5.5l8 2.5z';

  // Cada valor é a MARCAÇÃO INTERNA do <svg> (um ou mais <path>). fill herdado.
  var ICONS = {
    // ── base ──
    plane:   '<path d="' + PLANE + '"/>',
    ...
    // Meteo (set próprio, sólido — mapeado dos códigos met.no)
    ...
    // ── novos (a auditoria mostrou-os em falta; entram no canon, não ad-hoc) ──
  ...
  var MISSING = '<path fill="#E8482C" d="M2 2h20v20H2z"/><path fill="#fff" d="M11 6h2v7h-2zm0 9h2v2h-2z"/>';

  function icon(name, opts) {
    opts = opts || {};
    var inner = ICONS[name];
    if (inner == null) { inner = MISSING; if (window && window.console) console.warn('[CPIcons] nome inválido: ' + name); }
    if (opts.rot) inner = '<g transform="rotate(' + opts.rot + ' 12 12)">' + inner + '</g>';
    var size = opts.size ? opts.size : 20;
  ...
  function hydrate(root) {
    root = root || document;
    var nodes = root.querySelectorAll('[data-ic]');
    for (var i = 0; i < nodes.length; i++) {
      var el = nodes[i];
      var s = el.getAttribute('data-ic-size');
      var r = el.getAttribute('data-ic-rot');
      el.innerHTML = icon(el.getAttribute('data-ic'), { size: s ? +s : 0, rot: r ? +r : 0 });
    }
  }

  // CSS mínima: o host encolhe ao ícone; a cor vem do `color:` do contexto.
  function injectCss() {
    if (document.getElementById('cp-icons-css')) return;
    var st = document.createElement('style');
    st.id = 'cp-icons-css';
    st.textContent = '[data-ic]{display:inline-flex;align-items:center;justify-content:center;line-height:0;vertical-align:middle}[data-ic] svg{display:block}';
    document.head.appendChild(st);
  }

  if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', function () { injectCss(); hydrate(); });
  } else { injectCss(); hydrate(); }

  window.CPIcons = { icon: icon, hydrate: hydrate, names: Object.keys(ICONS) };
})();
