    ...
    return (
      <SafeAreaView style={s.safe} edges={['top']}>
        <ScrollView contentContainerStyle={[s.scroll, { paddingBottom: tabSpace }]} keyboardShouldPersistTaps="handled">
          ...
          <TouchableOpacity style={s.fcard} activeOpacity={0.8} onPress={() => navigation.navigate('FtlCalc', { code: 'CS FTL.1.205(g)' })}>
            <View style={s.badge}><Text style={s.badgeTxt}>205g</Text></View>
            <View style={{ flex: 1 }}>
              <Text style={s.fcardTitle} numberOfLines={2}>{t('ftl.calcDelayed', lang)}</Text>
              <Text style={s.fcardSub} numberOfLines={2}>{l('Apresentação adiada: que hora manda no PSV.', 'Delayed reporting: which time governs the FDP.')}</Text>
            </View>
            <Ionicons name="chevron-forward" size={16} color={C.sub} />
          </TouchableOpacity>
